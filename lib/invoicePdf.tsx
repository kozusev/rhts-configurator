...
function InvoiceDoc({ s, g, logo, hsCode, paymentTerms }: { s: OfferSnapshot; g: Record<string, string>; logo: string | null; hsCode: string; paymentTerms: string }) {
  ...
  if (s.package) items.push({ desc: `Milling pack — ${s.package.name}`, qty: 1, unit: s.package.price, amt: s.package.price });
  if (s.robot) items.push({ desc: `Robot — ${s.robot.label}`, qty: 1, unit: s.robot.price, amt: s.robot.price });
  for (const o of s.options) items.push({ desc: o.label, sub: o.sub, qty: o.qty, unit: o.unitPrice, amt: o.price });

  const cur = s.currency;
  const bankRows: [string, string][] = [
    ["Beneficiary", g.inv_bank_beneficiary],
    ["IBAN", g.inv_bank_iban],
    ["SWIFT / BIC", g.inv_bank_swift],
    ["Intermediary BIC", g.inv_bank_intermediary_bic],
    ["Beneficiary address", g.inv_bank_beneficiary_address],
    ["Bank / Payment institution", g.inv_bank_name],
    ["Bank address", g.inv_bank_address],
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.docBox}>
            ...
            {hsCode ? <Text style={styles.docMeta}>HS code: {hsCode}</Text> : null}
          </View>
        </View>
        ...
        <View style={styles.body}>
          <View style={styles.twoCol}>
            <View style={styles.col}>
              ...
              {g.inv_seller_tax_id ? <Text style={[styles.kv, styles.muted]}>Tax ID: {g.inv_seller_tax_id}</Text> : null}
          ...
          <View>
            ...
            <View style={styles.totRow}>
              <Text style={styles.totLabel}>Net total</Text>
              <Text style={styles.totValue}>{money(net, cur, s.locale)}</Text>
            </View>
            <View style={styles.totRow}>
              <Text style={styles.totLabel}>VAT {rate}%</Text>
